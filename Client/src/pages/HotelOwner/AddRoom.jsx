...
export default function AddRoom(){
    ...
    const[input, setInput] = useState({
        ...
        amenities: {
            ...
            'Room Service': false,
            'Moountain View': false,
            'Pool Access': false,
        }
    })
    return(
       <form>
        <Title align='left' font='poppins' title='Add Room' subTitle='Fill in the details carefully and accurate room details, pricing and amenities to enhance the user booking experience.'/>
       
        <p className="text-gray-800 mt-10">Images</p>
          <div className="grid grid-cols-2 sm:flex gap-4 my-2 flex-wrap">
           {Object.keys(images).map((key) => (
            <label htmlFor={`roomImage${key}`} key={key}>
                <img className="max-h-28 cursor-pointer opacity-80"
         ...
         <button className="bg-black text-white px-8 py-2 rounded mt-8 cursor-pointer mb-8">Add Room</button>
       </form>
    )
}